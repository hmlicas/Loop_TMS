...
class BaseModel:
    ...
    def load(self, sess):
        ...
        #latest_checkpoint = os.path.join(self.config.checkpoint_dir, self.config.exp_name) + "-80000"
        if latest_checkpoint:
            print("Loading model checkpoint {} ...\n".format(latest_checkpoint))
            self.saver.restore(sess, latest_checkpoint)
            print("Model loaded")

    # load pre-trained checkpoint from the experiment path defined in config_file
    def load_pretrain(self, sess):
        latest_checkpoint = tf.train.latest_checkpoint(self.config.checkpoint_dir_pretrain)
        if latest_checkpoint:
            print("Loading pretrain model checkpoint {} ...\n".format(latest_checkpoint))
            self.saver.restore(sess, latest_checkpoint)
            print("Model loaded")
